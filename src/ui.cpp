...
GameRenderer::GameRenderer(GameStats& stats)
    : gameStats(stats)
    , screenWidth(0)
    , screenHeight(0)
    , aspectRatio(0)
    ...
    , slotConfig(preset40Lines) // Temporarily
{
    ...
    DrawStatsFunctions = {{
        ...
        &GameRenderer::DrawScore,
        &GameRenderer::DrawTime,
        &GameRenderer::DrawLineCount,
        &GameRenderer::DrawPieces,
        &GameRenderer::DrawInputs,
        &GameRenderer::DrawFullClear
    }};

    UpdateScreenSize();
}

void GameRenderer::UpdateScreenSize()
{
...
void GameRenderer::CalculateElements()
{
    canvas.SetSize(
...
void GameRenderer::DrawHoldBox(const optional<Block>& holdBlock)
{
    holdBox.Draw(BLACK);
...
void GameRenderer::DrawQueueColumn(const deque<Block>& currentBag)
{
...
void GameRenderer::DrawBoard(const Board& board, const Block& currentBlock, int hardDropPos)
{
    BlockType type;
...
void GameRenderer::DrawStats()
{
    for (size_t i = 0; i < STAT_SLOT_COUNT; ++i)
        (this->*DrawStatsFunctions[slotConfig[i]])(i);
}

void GameRenderer::InvokeClearMsg(int clearedLine)
{
    string msg;
    switch (clearedLine)
    {
...
void GameRenderer::InvokeTSpinMsg(bool normalTspin)
{
    string msg = "t-spin";
    if (!normalTspin)
        msg = "mini " + msg;
...
void GameRenderer::InvokeB2BMsg(int b2bChain)
{
    messagesTimer[B2B_MSG] = ANIMATION_DURATION;
    messagesData[B2B_MSG] = format("back-to-back x{}", b2bChain);
}

void GameRenderer::InvokeComboMsg(int comboCount)
{
    messagesTimer[COMBO_MSG] = ANIMATION_DURATION;
    messagesData[COMBO_MSG] = format("{} COMBO", comboCount);
}

void GameRenderer::InvokeFullClearMsg()
{
    messagesTimer[FC_MSG] = ANIMATION_DURATION;
}

void GameRenderer::DrawStatSlot(int slotNumber, const string& slotText, const string& slotSubText)
{
...
void GameRenderer::DrawMessages(float frametime)
{
...
void GameRenderer::DrawLevel(int slotNumber)
{
    DrawStatSlot(slotNumber, format("{}", gameStats.level));
}

void GameRenderer::DrawScore(int slotNumber)
{
    DrawStatSlot(slotNumber, format("{}", gameStats.score));
}

void GameRenderer::DrawTime(int slotNumber)
{
    int minutes = static_cast<int>(gameStats.timeElapsed) / 60;
    int seconds = static_cast<int>(gameStats.timeElapsed) % 60;

    double fractionalSeconds = gameStats.timeElapsed - static_cast<int>(gameStats.timeElapsed);
...
void GameRenderer::DrawLineCount(int slotNumber)
{
    DrawStatSlot(slotNumber, format("{}", gameStats.clearedLineCount), "/40");
}

void GameRenderer::DrawPieces(int slotNumber)
{
    float pps = gameStats.droppedBlockCount / gameStats.timeElapsed;
    DrawStatSlot(slotNumber, format("{}, ", gameStats.droppedBlockCount), format("{:.2f}/s", pps));
}

void GameRenderer::DrawInputs(int slotNumber)
{
    float ips = 0;
    if (gameStats.droppedBlockCount)
        ips = 1.0 * gameStats.keyPressed / gameStats.droppedBlockCount;
    DrawStatSlot(slotNumber, format("{}, ", gameStats.keyPressed), format("{:.2f}/p", ips));
}

void GameRenderer::DrawFullClear(int slotNumber)
{
    DrawStatSlot(slotNumber, format("{}", gameStats.fullClearCount));
}
